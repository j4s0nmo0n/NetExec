...
try:
    ...
    # RFC 4121 values in case they are not exposed
...
# ---------------------------------------------------------------------------
# The GSSAPI_AES{128,256}.GSS_Wrap_LDAP and GSS_Unwrap_LDAP methods were
# only added to impacket starting in version 0.13. On older versions (0.11/0.12,
# still shipped with Parrot OS, Kali stable, etc.) we reimplement the same
# logic here from the primitives available in
# toutes les versions: WRAP(), cipherType(), rotate(), unrotate().
# RFC 4121 section 4.2.6.2: the AP-REP from the KDC provides the sub-key,
# RRC is 28 when encryption is requested.
...
def _gss_wrap_ldap_aes_compat(gss, sessionKey, data, sequenceNumber):
    ...
    and cipherText is the full encrypted+rotated payload.
    """
    token = gss.WRAP()
