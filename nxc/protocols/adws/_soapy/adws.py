...
class ADWSConnect:
    ...
    def _records_from_tree(
        self, et: ElementTree.Element, parse_values: bool = False
    ) -> list[dict]:
        """Convert an ADWS Pull response tree to a list of attribute dicts.

        Reuses the same parsing logic as ``_pretty_print_response`` (SidString,
        objectGUID, GeneralizedTimeString, time-since-1601 attributes,
        userAccountControl flags, etc.) but returns structured records instead
        of printing them. Each dict maps attribute name → string value (or
        comma-joined list for multi-valued attributes).

        Args:
            et: response xml element tree
            parse_values: when True, decode SIDs / GUIDs / timestamps / flags
                          into human-readable strings

        Returns:
            list of dicts, one per AD object in the response.
        """
        records: list[dict] = []
        for item in et.findall(".//ad:value/../..", namespaces=NAMESPACES):
            object_values: dict[str, str] = {}
            ...
            records.append(object_values)
        return records

    def get_rootdse_contexts(self) -> dict:
        """
        Retrieves the naming contexts and other RootDSE info.

        Uses a WS-Transfer ``Get`` request on the ``Resource`` endpoint
        avec l'objectGUID magique ``11111111-1111-1111-1111-111111111111``
        which designates the RootDSE object in ADWS. This is the **only**
        way to retrieve the RootDSE in ADWS - a Pull/Enumerate with
        ``basedn=""`` fails with "non-existent directory object".

        Retourne un dict avec :
          - ``defaultNamingContext`` (str)
          - ``schemaNamingContext`` (str)
          - ``configurationNamingContext`` (str)
          - ``rootDomainNamingContext`` (str)
          - ``namingContexts`` (list[str])
          - ``domainFunctionality`` (list[str])
          - ``forestFunctionality`` (str ou None)
          - ``dnsHostName`` (str ou None)

        This method requires the client to be in ``Resource`` mode.
        """
        if self._resource != "Resource":
            raise NotImplementedError(
                "get_rootdse_contexts requires a 'Resource' client; "
                "use ADWSConnect(..., resource='Resource') or .put_client()"
            )

        from xml.etree import ElementTree as ET
        from uuid import uuid4

        soap_request = LDAP_ROOT_DSE_FSTRING.format(
            uuid=str(uuid4()),
            fqdn=self._fqdn,
        )
        self._nmf.send(soap_request)
        response = self._nmf.recv()

        try:
            et = ET.fromstring(response)
        except Exception as e:
            raise RuntimeError(
                f"Error parsing RootDSE: {e}\nRaw response: {response[:500]}"
            )
        ...
        def getval(xpath):
            el = et.find(xpath, namespaces=ns)
            return el.text.strip() if el is not None and el.text else None

        def getvals(xpath):
            return [
                v.text.strip()
                for v in et.findall(xpath, namespaces=ns)
        ...
        return {
            ...
            "configurationNamingContext": getval(
                ".//addata:configurationNamingContext/ad:value"
            ),
            "rootDomainNamingContext": getval(
                ".//addata:rootDomainNamingContext/ad:value"
            ),
            ...
            "dnsHostName": getval(".//addata:dnsHostName/ad:value"),
        }
    ...
    def pull_records(
        ...
        """Same as ``pull()`` but returns a list of structured dicts instead of XML.

        Each dict maps attribute name to a string value (or comma-joined list
        for multi-valued attributes). Suitable for piping into custom
        formatters without re-parsing XML on the caller side.
        """
        et = self.pull(query, basedn, attributes, print_incrementally=False)
        return self._records_from_tree(et, parse_values=parse_values)
